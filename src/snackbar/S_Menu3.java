...
public class S_Menu3 extends JPanel {
	Sign_in sign_in;
	...
	public S_Menu3(Sign_in signin) {
		...
		JButton snack3point_charge_bt = new JButton("포인트충전");
		snack3point_charge_bt.setBounds(596, 31, 158, 55);
		this.add(snack3point_charge_bt);
		...
		JLabel snack3_point_label = new JLabel("잔여포인트 : " + signin.p.getC_vo().getPoint());
		snack3_point_label.setHorizontalAlignment(SwingConstants.CENTER);
		snack3_point_label.setBounds(284, 57, 158, 29);
		this.add(snack3_point_label);
...
snack3point_charge_bt.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				PointChargeDialog dialog = new PointChargeDialog((Frame) SwingUtilities.getWindowAncestor(S_Menu3.this));
		        dialog.setPointChargeListener(new PointChargeListener() {
		            public void onPointCharge(int amount) {
						...
						try {
		                    ...
		                    snack3_point_label.setText("잔여포인트 : " + signin.p.getC_vo().getPoint());
						} catch (Exception e2) {
							e2.printStackTrace();
						}
		            }
